...
int vooc_hardware_init_cp(int data)
{
	...
	if (data == 0 || data == 0x13 || data == 0x19 || data == 0x29 || data == 0x34 || data == 0x41
		...
		pr_debug("lkl CHARGER_SUBTYPE_FASTCHG_VOOC data:0x%x\n", data);
	...
	|| data == 0x14 || data == 0x61 || data == 0x66 || data == 0x69 || data == 0x6e) {
		...
		pr_debug("lkl CHARGER_SUBTYPE_FASTCHG_SVOOC data:0x%x\n", data);

		if (is_allow_fast_chg_real(g_vooc_chip)){
			oplus_chg_disable_charge();
			pr_debug("oplus_chg_disable_charge\n");
		}
		return CHARGER_SUBTYPE_FASTCHG_SVOOC;
	} else {
		pr_debug("lkl CHARGER_SUBTYPE_DEFAULT data:0x%x\n", data);
...
static void oplus_vooc_fastchg_func(struct work_struct *work)
{
	...
	if (chip->hiz_gnd_cable) {
		...
		pr_debug("temp_range[%d-%d-%d-%d-%d]", chip->vooc_low_temp, chip->vooc_little_cold_temp,
		...
		pr_debug(" firmware update, get fw_ver ready!\n");
		/*ready to get fw_ver*/
		fw_ver_info = 1;
		ret_info = 0x2;
		...
		pr_debug("local_fw:0x%x, need_to_up_fw:%d\n",
